...
void RRTStar::init(const std::vector<Eigen::Vector2f> &outside_cones, const std::vector<Eigen::Vector2f> &inside_cones, 
			const int start_index, const int end_index, const Eigen::Vector2f start_position, const Eigen::Vector2f end_position)
{
                ...
                if (isOnTrack(new_pos))
                {
                    ...
                    {   // trajectory cost optimization
                            ...
                            && (q_new->cost + pathCost(*q_new, *q_near)) < q_near->cost)
                        {
                            ...
                            q_parent->children.erase(std::remove(q_parent->children.begin(), q_parent->children.end(), q_near), q_parent->children.end());
...
void RRTStar::normalizePosition(const Node &q_nearest, Eigen::Vector2f *q_pos) const
{
    ...
    *q_pos = q_nearest.position + Eigen::Vector2f(cosf(angle) * conf_.node_step_size, sinf(angle) * conf_.node_step_size);
}
...
bool RRTStar::isOnTrack(const Eigen::Vector2f target_point) const
{
	...
	for (i = 0, j = out_cones_.size() - 1; i < out_cones_ .size(); j = i++) 
	{
		...
		((target_point(0) < (out_cones_ [j](0) - out_cones_ [i](0)) * (target_point(1) - out_cones_ [i](1)) / (out_cones_ [j](1) - out_cones_ [i](1)) + out_cones_ [i](0))) )
			c = !c;
	...
	if(c)
	{
		for (i = 0, j = in_cones_.size() - 1; i < in_cones_.size(); j = i++) 
		{
			...
			((target_point(0) < (in_cones_ [j](0) - in_cones_ [i](0)) * (target_point(1) - in_cones_ [i](1)) / (in_cones_ [j](1) - in_cones_ [i](1)) + in_cones_ [i](0))) )
				c = !c;
		}
